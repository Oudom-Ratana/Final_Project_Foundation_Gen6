...
export function CardStack({
  ...
  const actualCardWidth = containerWidth
    ...
    : cardWidth;
  const actualCardHeight = Math.round(
    actualCardWidth * (cardHeight / cardWidth),
  );
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-12 rounded-3xl overflow-hidden shadow-2xl transition-opacity duration-300",
                    "will-change-transform select-none",
                    isActive
                      ? "cursor-grab active:cursor-grabbing shadow-2xl shadow-black/90"
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on
                      ? "w-8 bg-[#B90101]"
                      : "w-2.5 bg-neutral-400/40 hover:bg-neutral-400/80 dark:bg-white/30",
                  )}
          ...
          {activeItem?.href &&
            (activeItem.href.startsWith("http") ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-neutral-400 hover:text-[#B90101] transition"
...
function DefaultFanCard({ item, active }) {
  return (
    <div className="relative h-full w-full bg-neutral-900 rounded-3xl overflow-hidden">
      {item.imageSrc ? (
        <img
          src={item.imageSrc}
          alt={item.title}
